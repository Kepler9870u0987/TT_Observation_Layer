...
async def create_new_version(
    ...
    Opera nella transazione corrente della sessione (gia aperta dal reader
    o da un begin esplicito nel chiamante) ed esegue commit al termine.

    Ritorna il nuovo dict_version_new = current_version + 1.
    """
    # Se la sessione non ha ancora una transazione attiva, la avviamo.
    if not db.in_transaction():
        await db.begin()

    # Advisory lock  rilasciato automaticamente al commit/rollback della transazione
    ...
    # Upsert lexicon_entries per regex_active e ner_active
    for action, entry in all_updates:
        ...
        await db.execute(
            text("""
                INSERT INTO lexicon_entries
                    (entry_id, label_id, dict_type, lemma, surface_forms,
                     ...
                     dict_version_added)
                VALUES
                    (:entry_id, :label_id, :dict_type, :lemma, CAST(:surface_forms AS jsonb),
            ...
            """),
            {
                ...
                "dict_type": dict_type,
                "lemma": lemma,
                "surface_forms": json.dumps(entry.get("surface_forms", [])),
    ...
    for entry in promoter_updates.get("quarantined", []):
        ...
        await db.execute(
            text("""
                ...
                VALUES
                    (:entry_id, :label_id, 'ner', :lemma, CAST(:surface_forms AS jsonb),
    ...
    for action, entry in all_events:
        await db.execute(
            text("""
                ...
                VALUES
                    (:event_id, :run_id, :label_id, :lemma, :dict_type, :action,
                     ...
                     :doc_freq, :emb_score, CAST(:collision_labels AS jsonb))
            """),
            {
                "event_id": str(uuid4()),
                "run_id": run_id,
                ...
                "collision_labels": json.dumps(entry.get("collision_labels", [])),
            },
        )

    # Marca le observations come promoted
    promoted_lemmas = [
        e["lemma"] for act, e in all_updates if act in ("regex_active", "ner_active")
    ]
    if promoted_lemmas:
        await db.execute(
            text("""
